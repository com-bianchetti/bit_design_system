...
You can create your own custom input widgets that integrate seamlessly with `VitForm`. By wrapping your widget's built-in state in a standard Flutter `FormField` and using the `VitFormProvider`, your field can take advantage of the same validation and data collection behaviors as native Vit components.

Here is an example inspired by the internal `VitCheckbox` component logic:
...
class _MyCustomFieldState extends State<MyCustomField> {
  ...
  Widget build(BuildContext context) {
    // 1. Wrap your custom component in a FormField
    return FormField<bool>(
      initialValue: _value,
      validator: widget.validator,
      // 2. Access VitFormProvider to save the value to the form state
      onSaved: (value) {
        if (widget.id != null) {
      ...
      // 3. Access field state (like validation errors) inside the builder
      builder: (field) {
        ...
        return Column(
          ...
          children: [
            // Your custom interactive UI
            CheckboxListTile(
              value: _value,
              title: const Text('Custom Checkbox'),
              onChanged: (newValue) {
                setState(() => _value = newValue ?? false);
                field.didChange(_value); // Trigger re-validation
              },
            ),
            // 4. Display validation errors if necessary
            if (hasError)
              Padding(
...
1. **`FormField<T>` Wrapper**: Use a standard `FormField` so the internal `Form` can trigger validation.
2. **Access `VitFormProvider`**: Inside the `onSaved` callback, access the provider with `VitFormProvider.maybeOf(context)`.
3. **Save Data**: Call `form?.save(id, value)` to register the final value with `VitForm` when the form completes.
4. **Error State**: Check `field.errorText` inside the `builder` to visually update your component if it fails validation.
